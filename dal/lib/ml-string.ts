...
const mlString = {
  ...
  resolve(
    ...
    // Try each priority language in order
    ...
    // Try fallbacks for the first priority language (backward compatibility)
    if (priorities.length > 0) {
      const fallbackLanguages = languages.getFallbacks(priorities[0]);
    ...
    // Final fallback: return first available language (for read language fallback)
  ...
  resolveWithMetadata(
    ...
    // Final fallback: return first available language (not in preferred list)
